...
export const SpeechFormatters = {
   ...
   */
  howItWorks(lang: 'en' | 'hi' = 'en'): string {
    if (lang === 'hi') {
   ...
   */
  problemDetails(issue: Issue, lang: 'en' | 'hi' = 'en'): string {
   ...
   */
  aiAnalysis(issue: Issue, aiTriage?: TriageResultResponse | null, lang: 'en' | 'hi' = 'en'): string {
   ...
   */
  journey(issue: Issue, lang: 'en' | 'hi' = 'en'): string {
    const stageNotes: string[] = [];

    // Stage 1: Citizen Reported
    stageNotes.push(lang === 'hi' 
   ...
   */
  govtReview(issue: Issue, lang: 'en' | 'hi' = 'en'): string {
    const loc = `${issue.location?.locality || ''}, ${issue.location?.district || 'Ramgarh'}, Jharkhand`;
    if (lang === 'hi') {
   ...
   */
  universityChallenge(issue: Issue, matchScore: number, reasons: string[], solutionDirection?: string, lang: 'en' | 'hi' = 'en'): string {
    if (lang === 'hi') {
   ...
   */
  csrOpportunity(issue: Issue, matchedLab: string, supportReq: string, lang: 'en' | 'hi' = 'en'): string {
    if (lang === 'hi') {
   ...
   */
  institutionRecommendation(matches: any[], lang: 'en' | 'hi' = 'en'): string {
    if (!matches || matches.length === 0) {
      return lang === 'hi' ? 'कोई अनुशंसित संस्थान उपलब्ध नहीं है।' : 'No recommended institution available.';
    }
    const top = matches[0];
   ...
   */
  similarProblemsList(similarProblems: any[], lang: 'en' | 'hi' = 'en'): string {
    if (!similarProblems || similarProblems.length === 0) {
      return lang === 'hi' ? 'झारखंड में कोई समान समस्या दर्ज नहीं है।' : 'No similar problems found in Jharkhand.';
    }
    const items = similarProblems.slice(0, 2).map(s => {
      ...
      return lang === 'hi' ? `${s.title}, ${score} प्रतिशत समान` : `${s.title}, ${score} percent similar`;
    }).join('. ');
   ...
   */
  nextActionStep(status: string, lang: 'en' | 'hi' = 'en'): string {
    if (lang === 'hi') {
